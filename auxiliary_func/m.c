...
int send_signal(int socketfd,int num) {
    char recv_buff[MAX_SIZE] = {0};
    char c[20] = {0};
    sprintf(c, "%d\n", num);
    if (send(socketfd, c, strlen(c), 0) < 0) {close(socketfd); return 0;}
    //if (recv(socketfd, recv_buff, MAX_SIZE, 0) < 0) {close(socketfd); return 0;}
    //if (!strcmp(recv_buff, "-1")) return 0;
    return 1;
}
...
int slave(int ip_count, int port, char **ip, int n) {
    ...
    if (socketfd > 0) {
        ...
        for (int i = 100; i <= 105; i++) {
            int flag = send_signal(socketfd, i);
            //printf("%d\n", flag);
            if (flag) {
